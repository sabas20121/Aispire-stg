...
const ConfigMenu = ({ open, setPanel }) => {
  ...
  return (
    <Box id="profile" className="Config-container">
      <List options={settings} open={open} onClick={onConfigMenuItemClick} />
      <Box
        className={`Config-copyright-container ${
          open ? '' : 'Config-copyright-container-closed'
        }`}
      >
        <img
          src={copyright}
      ...
      <ListItemIcon className="List-geticon-container">
        {
          <Avatar
            ...
            sx={{ left: '-4px', width: 20, height: 20 }}
          >
            <Typography className="Profile-avatar-text">{initials}</Typography>
          </Avatar>
        }
      </ListItemIcon>
      <ListItemText>
  ...
  return (
    <>
      <Box className={`LeftPanel ${open ? '' : 'LeftPanel-closed'}`}>
        <Branding open={open} />
        <MainMenu open={open} />
        <ConfigMenu open={open} setPanel={setPanel} />
